...
export default class PptGlobalsParser {
  ...
  public static async getGlobalSlideDimension(pptFilePath: string) {
    try {
      const slideShowGlobals = await FileHandler.parseContentFromFile(join(pptFilePath, "ppt/presentation.xml"));

      return getAttributeByPath(slideShowGlobals, ["p:presentation", "p:sldSz"], []);
    } catch (error) {
      throw error;
    }
  }
  ...
  public static async getSections(pptFilePath: string): Promise<PptSection[]> {
    try {
      ...
      let allSections: any[] = getAttributeByPath(
        slideShowGlobals,
        ["p:presentation", "p:extLst", "p:ext", "p14:sectionLst", "p14:section"],
        []
      );
